...
//        BM25Similarity bm25similarity = new BM25Similarity();
        ...
        try {
            ...
            final List<Future<Document>> result = executor.invokeAll(list);
            ...
            new Thread(new Runnable() {
				...
				public void run() {
					try {
						IndexWriterConfig indexFilewriterConfig = new IndexWriterConfig(Version.LUCENE_4_9, standardanalyzer);
						indexFilewriterConfig.setSimilarity(tfidfSimilarity);
						Directory fileDirectory = new SimpleFSDirectory(new File("/home/ninja/index/" + new SimpleDateFormat("hhmm_ddMMyyyy").format(new Date())));
						IndexWriter indexwriter;
						indexwriter = new IndexWriter(fileDirectory, indexFilewriterConfig);
						for (Future<Document> document : result) {
							indexwriter.addDocument(document.get());
						}
						indexwriter.close();
